...
/***************************************************************************/
/***************************************************************************/
_Bool kbi_cmd(uint8_t fc, uint8_t cmd, uint8_t *pld, uint16_t pldLen,UART_HandleTypeDef huart1)
{
	uint8_t retries = KBI_CMD_RETRIES;
	while (retries)
	{
		cmds_send(CMDS_FTCMD | fc, cmd, pld, pldLen, huart1);
...
_Bool kbi_waitFor(uint8_t cmd, uint8_t *pld, uint16_t len, uint16_t tout, UART_HandleTypeDef huart1)
{
	uint32_t end = tout;
	uint32_t time = 0;
	while (time < end)
	{
		if (kbi_cmd(CMDS_FCCMD_READ, cmd, NULL, 0, huart1))
		{
